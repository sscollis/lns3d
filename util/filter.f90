!============================================================================!
        subroutine filter_x( nx, ny, f )
!  
!  Filter a 2-D field in x
!  
!============================================================================!
        implicit none
        
        integer:: nx, ny
        real :: f(nx,ny), rhs(nx,ny)
...
!.... setup RHS of filter in x
        ...
        rhs(3,:) = 5.0/8.0 * f(3,:) + (-f(1,:) + 4.0 * f(2,:) + &
                   4.0 * f(4,:) - f(5,:))/16.0
        rhs(4:nx-3,:) = a * f(4:nx-3,:) + &
                        b/2.0 * ( f(5:nx-2,:) + f(3:nx-4,:) ) + &
                        c/2.0 * ( f(6:nx-1,:) + f(2:nx-5,:) ) + &
                        d/2.0 * ( f(7:nx,:)   + f(1:nx-6,:) )
        rhs(nx-2,:) = 5.0/8.0 * f(nx-2,:) + (-f(nx-4,:) + 4.0 * f(nx-3,:) + &
                      4.0 * f(nx-1,:) - f(nx,:))/16.0

        rhs(nx-1,:) = 3.0/4.0 * f(nx-1,:) + (f(nx,:) + 6.0 * f(nx-2,:) - &
                      4.0 * f(nx-3,:) + f(nx-4,:))/16.0
        rhs(nx,:) = (15.0 * f(nx,:) + 4.0 * f(nx-1,:) - 6.0 * f(nx-2,:) + &
                    4.0 * f(nx-3,:) - f(nx-4,:))/16.0

!.... setup LHS of filter in x

        if (beta.eq.0.0) then           !.... tridiagonal LHS

          allocate ( lhs(nx,ny,3) )
...
!============================================================================!
        subroutine filter_y( nx, ny, f )
!  
!  Filter a 2-D field in y
!  
!============================================================================!
        implicit none
...
!=============================================================================!
        subroutine solve1( nsys, n, nblk, mat, rhs )
!=============================================================================!
!  
...
!=============================================================================!
        ...
        real, intent(inout) :: mat(nsys,n,nblk,nblk*3), rhs(nsys,n,nblk)
