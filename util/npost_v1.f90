...
!.... potential flow stuff and boundary layer stuff
        ...
        real :: c1, Ma1, Cp, phip, Re1, ReL, L, L1, L2, L3, &
                betah, beta2, beta3, H, rk, p0, pinf, dusds
!============================================================================!
...
!.... 0.999 point in W_c
 ...
 700      edge = RTSAFE( fedge, ynn(j-1), ynn(j+2), 1.0e-14 )
        ...
        close(11)
        deallocate ( yknot, bs, ynn, uss, unn )
        end if
        end if
...
!.... beta.dat for NSBL5 solver
          ...
          write(15,12) s(i), uss(i), dusds, betah, zero, zero, x(1,i), &
                       y(1,i), bn1, bn2
...
!       if (kyord.eq.0) call exit(0)
        ...
        allocate ( yknot(ny+kyord), bs(ny,ndof), ynn(ny), &
...
!         write(*,"(9(1pe13.6,1x))") s(i), U1, ue, we, phie, cos(alp) * U1
          ...
          call BSINT( ny, ynn, uss, kyord, yknot, bs(:,2) )
          call BSINT( ny, ynn, wss, kyord, yknot, bs(:,4) )
...
!.... determine the location of maximum crossflow
 ...
 75       if (j.eq.ny) then
          ...
          wmax  = BSDER( 0, wmloc, kyord, yknot, ny, bs(:,4) )

!.... find the inflection point in the crossflow profile

          do j = jmax, ny-1
            w1 = BSDER( 2,   ynn(j), kyord, yknot, ny, bs(:,4) )
            w2 = BSDER( 2, ynn(j+1), kyord, yknot, ny, bs(:,4) )
 ...
 80       if (j.eq.ny) then
          ...
          uinf  = BSDER( 0, wiloc, kyord, yknot, ny, bs(:,2) )
          winf  = BSDER( 0, wiloc, kyord, yknot, ny, bs(:,4) )
...
!.... output a profile
          ...
          if(.false.) then
            do j = 1, ny
              ...
              u3  = BSDER( 0, ynn(j), kyord, yknot, ny, bs(:,4) )
...
!....   Beta_H = 2 m / (m + 1)
        ...
        deallocate (yknot, bs, ynn, uss, unn, wss, uel)
...
!=============================================================================!
        rho = BSDER( 0, x, kyord, yknot, ny, bs(:,1) )
        w   = BSDER( 0, x, kyord, yknot, ny, bs(:,4) )
...
!.... Delta 0.999 in W_c
        ...
        g = BSDER( 0, x, kyord, yknot, ny, bs(:,4) ) - 0.999*tan(alpha)
        d = BSDER( 1, x, kyord, yknot, ny, bs(:,4) )

!.... Delta 0.999 in T
        
!       g = BSDER( 0, x, kyord, yknot, ny, bs(:,5) ) - 0.999
!       d = BSDER( 1, x, kyord, yknot, ny, bs(:,5) )
...
!=============================================================================!
        ...
        real :: x, f, g, d
!=============================================================================!
        f = BSDER( 0, x, kyord, yknot, ny, bs(:,4) )
        g = BSDER( 1, x, kyord, yknot, ny, bs(:,4) )
        d = BSDER( 2, x, kyord, yknot, ny, bs(:,4) )
...
!=============================================================================!
        ...
        real :: x, f, g, d
!=============================================================================!
        f = BSDER( 1, x, kyord, yknot, ny, bs(:,4) )
        g = BSDER( 2, x, kyord, yknot, ny, bs(:,4) )
        d = BSDER( 3, x, kyord, yknot, ny, bs(:,4) )
