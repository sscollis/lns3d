#!/bin/bash
set -e
\rm -f output.* lns.dat wall.dat *.pot grid.dat grid.xyz lns.q.* *.log
...
\rm -f pedge.dat top.dat profile.* first.* second.* eig.pro *.dat *.out space.1
\rm -f dke.tex plot.aux plot.pdf
exit $? 
