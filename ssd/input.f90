...
!       read (*,*) mattyp
        ...
        if (mattyp .eq. 1) then              ! Sutherland's law 
!         write (*,"('Enter Pinf (Pa) , Tinf (K) ==> ',$)")
          read (*,*) Pref, Tref
          mu0    = 1.715336725523065e-05     ! (AIR)
          T0     = 273.0
          S0     = 110.4
          Rgas   = 286.6
...
!       3 = REMOVED:  Poinsot & Lele wall BC (not implemented in LHS)
!       4 = Poinsot & Lele continuity equation at the wall (in LHS too)
!
!.... Wall temperature BC flag
!
!       0 = isothermal wall
...
!.... some sanity checks
        ...
        if (eps_e .ne. zero) then
          if (xper .or. yper) then
            call error(code,'Periodic BC not supported with eps_e .ne. 0$')
          else if (tsym .or. bsym) then
            call error(code,'tsym/bsym not supported with eps_e .ne. 0$')
          end if
        end if
