...
!.... implicit problems
        ...
        if (linear.eq.1 .or. impl.ne.0) then
          ...
          allocate( Q1(ny*nx,ndof,ndof), Q2(ny*nx,ndof,ndof),   &
                    Q3(ny*nx,ndof,ndof), STAT=ier )
          if (ier .ne. 0) call error(code,'Insufficient Memory$')
  
          mem = mem + ny*nx*4
          allocate( Vij(ny*nx,4), STAT=ier )
          if (ier .ne. 0) call error(code,'Insufficient Memory$')
  
          mem = mem + ny*nx*12
          allocate( Vh1(ny*nx,6), Vh2(ny*nx,6), STAT=ier )
          if (ier .ne. 0) call error(code,'Insufficient Memory$')
        end if
...
!.... deallocate stuff not needed for linear analysis
          ...
          if (kz .eq. zero .and. omega .eq. zero) then
            mem = mem + ny*nx*ndof*ndof
            deallocate( Q3 )
            write(*,*) 'Q3 is deallocatted'
          end if
          mem = mem + ny*nx*4
          deallocate( Vij )
