!=============================================================================!
        subroutine lhs2f3D( mat, Q1, Q2, Q3, Vh1, Vh2, dtl, v)
!  
...
!.... fourth-order stencil
        ...
        lrec = ny * nx * 6
        call readdr(MATRIX, Vh2, lrec, 7, ier)          ! \hat{B}_i
...
!.... \hat{B}_i term
        ...
        mat(:,2,4,1) = mat(:,2,4,1) - c1 * dtl(:) * Vh2(:,3)
        mat(:,3,4,1) = mat(:,3,4,1) - c1 * dtl(:) * Vh2(:,4)
        mat(:,4,2,1) = mat(:,4,2,1) - c1 * dtl(:) * Vh2(:,3)
        mat(:,4,3,1) = mat(:,4,3,1) - c1 * dtl(:) * Vh2(:,4)

        mat(:,2,4,2) = mat(:,2,4,2) - c2 * dtl(:) * Vh2(:,3)
        mat(:,3,4,2) = mat(:,3,4,2) - c2 * dtl(:) * Vh2(:,4)
        mat(:,4,2,2) = mat(:,4,2,2) - c2 * dtl(:) * Vh2(:,3)
        mat(:,4,3,2) = mat(:,4,3,2) - c2 * dtl(:) * Vh2(:,4)

        mat(:,2,4,3) = mat(:,2,4,3) - c3 * dtl(:) * Vh2(:,3)
        mat(:,3,4,3) = mat(:,3,4,3) - c3 * dtl(:) * Vh2(:,4)
        mat(:,4,2,3) = mat(:,4,2,3) - c3 * dtl(:) * Vh2(:,3)
        mat(:,4,3,3) = mat(:,4,3,3) - c3 * dtl(:) * Vh2(:,4)

        mat(:,2,4,4) = mat(:,2,4,4) - c4 * dtl(:) * Vh2(:,3)
        mat(:,3,4,4) = mat(:,3,4,4) - c4 * dtl(:) * Vh2(:,4)
        mat(:,4,2,4) = mat(:,4,2,4) - c4 * dtl(:) * Vh2(:,3)
        mat(:,4,3,4) = mat(:,4,3,4) - c4 * dtl(:) * Vh2(:,4)

        mat(:,2,4,5) = mat(:,2,4,5) - c5 * dtl(:) * Vh2(:,3)
        mat(:,3,4,5) = mat(:,3,4,5) - c5 * dtl(:) * Vh2(:,4)
        mat(:,4,2,5) = mat(:,4,2,5) - c5 * dtl(:) * Vh2(:,3)
        mat(:,4,3,5) = mat(:,4,3,5) - c5 * dtl(:) * Vh2(:,4)
...
!.... \hat{V}_{\eta\eta} and I terms
        ...
        call lhs2lf3D( mat, Vh1, dtl, buff, calcd )

!.... apply boundary treatment to the LHS

        call lhsbt2f3D( mat, Q1, Q2, Q3, Vh1, Vh2, spg, spg2, dtl, calcd )
...
!=============================================================================!
        subroutine lhs2lf3D( mat, Vh1, dtl, buff, calcd )
!  
