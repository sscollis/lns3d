...
!=============================================================================!
	...
	do i = 1, nx
	  do j = 1, ny
	    resnod = ( res(j,i,1) * conjg(res(j,i,1)) + &
                       res(j,i,2) * conjg(res(j,i,2)) + &
		       res(j,i,3) * conjg(res(j,i,3)) + &
		       res(j,i,4) * conjg(res(j,i,4)) + &
		       res(j,i,5) * conjg(res(j,i,5)) ) / dtl(j,i)**2
	...
	if (ixrmax.ne.0 .and. iyrmax.ne.0) then
	  resmx(:) = abs(res(iyrmax,ixrmax,:)) / dtl(iyrmax,ixrmax)
	else
	  resmx(:) = zero
	end if
