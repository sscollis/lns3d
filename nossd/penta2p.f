!=============================================================================!
        subroutine penta2p( np, nsys, nblk, mat, rhs, per, per2, mult, fact )
!=============================================================================!
!  
...
!=============================================================================!
        ...
        real mat(np,nsys,nblk,nblk,5), rhs(np,nsys,nblk)
        real per(np,nsys,nblk,nblk,2), per2(nsys,nblk,nblk,6)
...
!.... setup the periodic fillin vector
        ...
        do q = 1, nblk
          do m = 1, nblk
            do iv = 1, nsys
              per(1,iv,m,q,1) = mat(1,iv,m,q,1)
              per(1,iv,m,q,2) = mat(1,iv,m,q,2)
              per(2,iv,m,q,1) = zero
              per(2,iv,m,q,2) = mat(2,iv,m,q,1)
              
              per2(iv,m,q,1)  = mat(n-1,iv,m,q,5)
              per2(iv,m,q,2)  = zero
              per2(iv,m,q,4)  = mat(n,iv,m,q,4)
              per2(iv,m,q,5)  = mat(n,iv,m,q,5)
            end do
          end do
        end do
...
!.... the periodic stuff on the next-to-the-last row
            ...
            do q = nc + 1, nblk
              do iv = 1, nsys
                per2(iv,m,q,1) = per2(iv,m,q,1) + mult(iv) * mat(i,iv,nc,q,3)
              end do
            end do
            do q = 1, nblk
              do iv = 1, nsys
                per2(iv,m,q,2) = per2(iv,m,q,2) + mult(iv) * mat(i,iv,nc,q,4)
                per2(iv,m,q,3) = mult(iv) * mat(i,iv,nc,q,5)
...
!.... the periodic stuff on the last row
            ...
            do q = nc + 1, nblk
              do iv = 1, nsys
                per2(iv,m,q,4) = per2(iv,m,q,4) + mult(iv) * mat(i,iv,nc,q,3)
              end do
            end do
            do q = 1, nblk
              do iv = 1, nsys
                per2(iv,m,q,5) = per2(iv,m,q,5) + mult(iv) * mat(i,iv,nc,q,4)
                per2(iv,m,q,6) = mult(iv) * mat(i,iv,nc,q,5)
...
!.... the periodic stuff on the next-to-the-last row
              ...
              do q = 1, nblk
                do iv = 1, nsys
                  per2(iv,m,q,2) = per2(iv,m,q,2) + mult(iv) * mat(i,iv,nc,q,4)
                  per2(iv,m,q,3) = per2(iv,m,q,3) + mult(iv) * mat(i,iv,nc,q,5)
                  mat(n-1,iv,m,q,3) = mat(n-1,iv,m,q,3) + mult(iv) * per(i,iv,nc,q,1)
                  mat(n-1,iv,m,q,4) = mat(n-1,iv,m,q,4) + mult(iv) * per(i,iv,nc,q,2)
                end do
              end do
...
!.... the periodic stuff on the last row
              ...
              do q = 1, nblk
                do iv = 1, nsys
                  per2(iv,m,q,5) = per2(iv,m,q,5) + mult(iv) * mat(i,iv,nc,q,4)
                  per2(iv,m,q,6) = per2(iv,m,q,6) + mult(iv) * mat(i,iv,nc,q,5)
                  mat(n,iv,m,q,2) = mat(n,iv,m,q,2) + mult(iv) * per(i,iv,nc,q,1)
                  mat(n,iv,m,q,3) = mat(n,iv,m,q,3) + mult(iv) * per(i,iv,nc,q,2)
                end do
              end do
          ...
          do q = 1, nblk
            do m = 1, nblk
              do iv = 1, nsys
                per2(iv,m,q,1)  = per2(iv,m,q,2)
                per2(iv,m,q,2)  = per2(iv,m,q,3)
                per2(iv,m,q,4)  = per2(iv,m,q,5)
                per2(iv,m,q,5)  = per2(iv,m,q,6)
              end do
            end do
          end do
