!=============================================================================!
        subroutine solve1p( nsys, np, nblk, mat, rhs, per, per2 )
!=============================================================================!
!  
...
!=============================================================================!
        ...
        real, intent(inout) :: mat(3,nblk,nblk,nsys,np), rhs(nblk,nsys,np)
        real, intent(inout) :: per(nblk,nblk,nsys,np), per2(2,nblk,nblk,nsys)
          ...
          do nc = 1, nblk - 1
            ...
            do m = nc, nblk - 1
              ...
              do q = 1, nblk
                ...
                per(l,q,iv,i) = per(l,q,iv,i) + mult * per(nc,q,iv,i)
              end do
...
!.... for all columns
          ...
          do m = 1, nblk
            ...
            do q = 1, nblk
              ...
              per(m,q,iv,p) = mult * per(nc,q,iv,i)
            end do
...
!.... the periodic stuff on the last row
            ...
            do q = nc + 1, nblk
              per2(1,m,q,iv) = per2(1,m,q,iv) + mult * mat(2,nc,q,iv,i)
            end do
            do q = 1, nblk
              per2(2,m,q,iv) = mult * mat(3,nc,q,iv,i)
              mat(2,m,q,iv,n) = mat(2,m,q,iv,n) + mult * per(nc,q,iv,i)
            end do
          ...
          do nc = 2, nblk
            ...
            do m = 1, nblk
              ...
              do q = 1, nblk
                mat(2,m,q,iv,p) = mat(2,m,q,iv,p) + mult * mat(3,nc,q,iv,i)
                per(m,q,iv,p) = per(m,q,iv,p) + mult * per(nc,q,iv,i)
              end do
              rhs(m,iv,p) = rhs(m,iv,p) + mult * rhs(nc,iv,i)
...
!.... the periodic stuff on the last row
              ...
              do q = 1, nblk
                per2(2,m,q,iv) = per2(2,m,q,iv) + mult * mat(3,nc,q,iv,i)
                mat(2,m,q,iv,n) = mat(2,m,q,iv,n) + mult * per(nc,q,iv,i)
              end do
              rhs(m,iv,n) = rhs(m,iv,n) + mult * rhs(nc,iv,i)
            end do
...
!.... for all columns
        ...
        do nc = 1, nblk
          ...
          do m = 1, nblk
            ...
            do q = 1, nblk
              mat(2,m,q,iv,p) = mat(2,m,q,iv,p) + mult * mat(3,nc,q,iv,i)
              mat(3,m,q,iv,p) = mat(3,m,q,iv,p) + mult * per(nc,q,iv,i)
            end do
            rhs(m,iv,p) = rhs(m,iv,p) + mult * rhs(nc,iv,i)
...
!.... the periodic stuff on the last row
            ...
            do q = 1, nblk
              mat(1,m,q,iv,n) = mat(1,m,q,iv,n) + mult * mat(3,nc,q,iv,i)
              mat(2,m,q,iv,n) = mat(2,m,q,iv,n) + mult * per(nc,q,iv,i)
            end do
            rhs(m,iv,n) = rhs(m,iv,n) + mult * rhs(nc,iv,i)
          end do
