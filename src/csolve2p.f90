!=============================================================================!
        subroutine csolve2p( np, nsys, nblk, mat, rhs, per, per2, mult, fact)
!=============================================================================!
!  
...
!=============================================================================!
        ...
        complex, intent(inout) :: mat(3,nblk,nblk,np,nsys), rhs(nblk,np,nsys)
        complex, intent(inout) :: per(nblk,nblk,np,nsys)
...
!.... the periodic stuff on the last row
            ...
            do q = nc + 1, nblk
              per2(1,m,q,iv) = per2(1,m,q,iv) + mult(iv) * mat(2,nc,q,i,iv)
            end do
            do q = 1, nblk
              per2(2,m,q,iv) = mult(iv) * mat(3,nc,q,i,iv)
              mat(2,m,q,n,iv) = mat(2,m,q,n,iv) + mult(iv) * per(nc,q,i,iv)
            end do
          ...
          do nc = 2, nblk
            ...
            do m = 1, nblk
              ...
              do q = 1, nblk
                mat(2,m,q,p,iv) = mat(2,m,q,p,iv) + mult(iv) * mat(3,nc,q,i,iv)
                per(m,q,p,iv) = per(m,q,p,iv) + mult(iv) * per(nc,q,i,iv)
              end do
              rhs(m,p,iv) = rhs(m,p,iv) + mult(iv) * rhs(nc,i,iv)
...
!.... the periodic stuff on the last row
              ...
              do q = 1, nblk
                per2(2,m,q,iv) = per2(2,m,q,iv) + mult(iv) * mat(3,nc,q,i,iv)
                mat(2,m,q,n,iv) = mat(2,m,q,n,iv) + mult(iv) * per(nc,q,i,iv)
              end do
              rhs(m,n,iv) = rhs(m,n,iv) + mult(iv) * rhs(nc,i,iv)
            end do
...
!.... for all columns
        ...
        do nc = 1, nblk
          ...
          do m = 1, nblk
            ...
            do q = 1, nblk
              mat(2,m,q,p,iv) = mat(2,m,q,p,iv) + mult(iv) * mat(3,nc,q,i,iv)
              mat(3,m,q,p,iv) = mat(3,m,q,p,iv) + mult(iv) * per(nc,q,i,iv)
            end do
            rhs(m,p,iv) = rhs(m,p,iv) + mult(iv) * rhs(nc,i,iv)
...
!.... the periodic stuff on the last row
            ...
            do q = 1, nblk
              mat(1,m,q,n,iv) = mat(1,m,q,n,iv) + mult(iv) * mat(3,nc,q,i,iv)
              mat(2,m,q,n,iv) = mat(2,m,q,n,iv) + mult(iv) * per(nc,q,i,iv)
            end do
            rhs(m,n,iv) = rhs(m,n,iv) + mult(iv) * rhs(nc,i,iv)
          end do
